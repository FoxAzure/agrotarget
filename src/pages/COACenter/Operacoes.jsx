...
const Operacoes = () => {
  ...
  // --- 1. DATAS DISPONÍVEIS (TRATADAS DD/MM/AAAA) ---
  ...
  return (
    <div className="min-h-screen bg-[#0B0F1A] text-slate-300 font-sans pb-10">
      ...
      <main className="p-4 flex flex-col items-center">
          ...
          <div className="bg-[#161B22] rounded-2xl border border-slate-800 shadow-2xl p-6">
            ...
            <div className="flex flex-col gap-4">
              <div className="flex flex-col gap-1">
                ...
                <select 
                  value={selectedDate} 
                  onChange={(e) => { setSelectedDate(e.target.value); handleReset(); }}
              ...
              <div className="relative">
                <input 
                  ...
                  className="w-full bg-[#0D1117] border border-slate-700 text-slate-200 font-bold rounded-lg pl-3 pr-10 py-2.5 outline-none focus:border-emerald-500 transition-colors placeholder:text-slate-600"
                />
                {!selectedEntity && searchTerm.length >= 2 && (
                  <div className="absolute top-full left-0 w-full mt-2 bg-[#161B22] border border-slate-700 rounded-lg shadow-2xl overflow-hidden z-50">
                    {filteredOptions.map((opt, idx) => (
                      <button key={idx} onClick={() => handleSelectOption(opt)} className="w-full text-left px-4 py-3 text-xs font-bold text-slate-400 hover:bg-[#1C2128] hover:text-emerald-400 border-b border-slate-800 last:border-0 transition-colors">
                        {opt}
                      </button>
                    ))}
                  </div>
        ...
        {/* 2. LISTAGEM DE EQUIPAMENTOS DARK */}
        {selectedEntity && entityData && (
          <div className="w-full max-w-md flex flex-col gap-6 animate-in slide-in-from-bottom-4">
            ...
            {Object.entries(entityData.equipamentos).map(([eqFullName, eqData], eqIdx) => (
              <div key={eqIdx} className="bg-[#161B22] rounded-2xl border border-slate-800 shadow-xl overflow-hidden">
                <div className="bg-[#0D1117] p-4 border-b border-slate-800">
                  <span className="text-[8px] font-black text-emerald-500 uppercase tracking-widest leading-none">Equipamento</span>
                        ...
                        <div key={grpIdx} className={`rounded-xl border transition-all ${isOpen ? 'border-slate-700 bg-[#0D1117]' : 'border-transparent bg-[#1C2128]'}`}>
                          ...
                          >
                            <div className="flex items-center gap-3">
                              ...
                              <span className={`text-[11px] font-black uppercase tracking-tight ${isProd ? 'text-emerald-400' : 'text-slate-400'}`}>
                                {grpName}
                              </span>
                            </div>
                            <span className={`text-xs font-black ${isProd ? 'text-emerald-500' : 'text-slate-500'}`}>
                              {secondsToTime(grpData.totalSecs)}
                            </span>
                          </button>
                            ...
                            <div className="px-4 pb-3 flex flex-col gap-1 border-t border-slate-800/50 pt-2">
                              {Object.entries(grpData.operacoes)
                                ...
                                <div key={opIdx} className="flex justify-between items-center py-2 border-b border-slate-800/30 last:border-0">
                                  <span className={`text-[11px] font-medium ${isProd ? 'text-emerald-300' : 'text-slate-500'}`}>
                                    {opName}
                                  </span>
            ...
            {/* 3. RESUMO GERAL DARK */}
            <div className="bg-[#161B22] rounded-2xl border border-slate-800 shadow-2xl p-6">
              <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] mb-5 border-b border-slate-800 pb-3">
