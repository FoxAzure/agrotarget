...
const RESUMO_COLUMNS = [
  'data', 'semana', 'mes', 'ano', 'desc_area', 'categoria',
  'hrs_total_seg', 'hrs_disp_seg', 'hrs_produtivas_seg',
  'hrs_s_apont_seg', 'hrs_indeter_seg', 'hrs_manutencao_seg',
  'hrs_deslocamento_seg', 'hrs_total', 'hrs_disp',
  'hrs_produtivas', 'hrs_s_apont', 'hrs_indeter',
  'hrs_manutencao', 'hrs_deslocamento', 'ef_op',
  'ef_real', 'perc_s_apont', 'perc_indeter', 'disp_mec',
].join(',');
...
const aggregateSummaryRows = (rows = []) => {
  const acc = rows.reduce(
    ...
    { hrs_total_seg: 0, hrs_disp_seg: 0, hrs_produtivas_seg: 0, hrs_s_apont_seg: 0, hrs_indeter_seg: 0, hrs_deslocamento_seg: 0 }
  );
  ...
  return {
    ...acc, ef_op, ef_real, perc_s_apont, perc_indeter, disp_mec,
    hrs_produtivo_liquido_seg, hrs_restante_seg,
  };
};
      ...
      <div className="flex flex-wrap gap-2">
        {categoryOptions.map((category) => {
          ...
          return (
            ...
            >
              <input type="checkbox" checked={checked} onChange={() => onToggle(category)} className="hidden" />
              <span>{category}</span>
            </label>
          );
        })}
...
const AreaFilterTable = ({ rows = [], selectedAreas = [], onToggleArea }) => (
  <div className="coa-panel p-0 overflow-hidden coa-area-table-home">
    <div className="grid grid-cols-[1.5fr_0.5fr_0.6fr_0.7fr] gap-2 px-4 py-2.5 border-b" style={{ borderColor: 'var(--coa-divider)' }}>
      ...
      {rows.length === 0 ? (
        ...
        rows.map((row) => {
          ...
          return (
            ...
            >
              <span className="text-[12px] font-black text-[var(--coa-text)] truncate pr-2">{row.desc_area}</span>
              <span className="text-[12px] font-black text-right text-[var(--coa-text-soft)]">{formatInt(row.qnt_equip)}</span>
              <span className="text-[12px] font-black text-right" style={{ color: getEfColor(row.ef_op) }}>
                {formatPercent(row.ef_op)}
              </span>
              <span className="text-[12px] font-black text-right" style={{ color: getKpiColor('sapont', row.perc_s_apont) }}>
...
const MetricCard = ({ label, value, color = 'var(--coa-text)' }) => (
  <div className="rounded-[14px] border px-3 py-3 bg-[rgba(255,255,255,0.02)]" style={{ borderColor: 'var(--coa-divider)' }}>
    <span className="block text-[10px] font-black uppercase tracking-[0.14em] text-[var(--coa-text-muted)] mb-1">{label}</span>
    <span className="block text-[1.02rem] md:text-[1.08rem] font-black tracking-tight leading-none" style={{ color }}>{value}</span>
  </div>
);
  ...
  return (
    <div className="coa-panel p-4 flex flex-col items-center justify-center gap-3">
      ...
      <div
        ...
        style={{ background: `conic-gradient(${color} 0deg ${angle}deg, rgba(255,255,255,0.08) ${angle}deg 360deg)` }}
      >
        <div className="absolute inset-[14px] rounded-full border flex items-center justify-center bg-[rgba(3,8,20,0.96)]" style={{ borderColor: 'var(--coa-divider)' }}>
          <div className="flex flex-col items-center justify-center">
            <span className="text-[10px] font-black uppercase tracking-[0.14em] text-[var(--coa-text-muted)]">Ef.Op.</span>
            <span className="text-[1.28rem] font-black tracking-tight" style={{ color }}>{formatPercent(safeValue)}</span>
          </div>
        </div>
      </div>
      <span className="text-[11px] font-bold text-[var(--coa-text-soft)] text-center">Produtivas / Disponíveis</span>
    </div>
  );
};

const RealEfficiencyCard = ({ value }) => (
  <div className="rounded-[14px] border px-3 py-3 bg-[rgba(255,255,255,0.015)]" style={{ borderColor: 'var(--coa-divider)' }}>
    <span className="block text-[10px] font-black uppercase tracking-[0.14em] text-[var(--coa-text-muted)] mb-1">Eficiência Real</span>
    <span className="block text-[1rem] font-black tracking-tight text-[var(--coa-text)]">{formatPercent(value)}</span>
    <span className="block mt-1 text-[11px] font-bold text-[var(--coa-text-soft)]">Produtivas / Totais</span>
  </div>
);
...
const BreakdownBar = ({ totalSeg = 0, deslocSeg = 0, productiveNetSeg = 0, restanteSeg = 0 }) => {
  ...
  return (
    <div className="rounded-[14px] border p-3 bg-[rgba(255,255,255,0.015)]" style={{ borderColor: 'var(--coa-divider)' }}>
      <div className="flex items-center justify-between gap-3 mb-3">
        <span className="text-[10px] font-black uppercase tracking-[0.14em] text-[var(--coa-text-muted)]">Detalhamento Horas Produtivas</span>
        <span className="text-[11px] font-bold text-[var(--coa-text-soft)]">Total: {formatHHMMFromSeconds(totalSeg)}</span>
      </div>
      <div className="w-full h-[14px] rounded-full overflow-hidden border bg-[rgba(255,255,255,0.05)]" style={{ borderColor: 'var(--coa-divider)' }}>
        <div className="w-full h-full flex">
          <div title={`Deslocamento: ${formatHHMMFromSeconds(deslocSeg)}`} style={{ width: `${clampPercent(deslocPct)}%`, background: '#14532d' }} />
          <div title={`Produtivo: ${formatHHMMFromSeconds(productiveNetSeg)}`} style={{ width: `${clampPercent(productivePct)}%`, background: '#22c55e' }} />
          <div title={`Restante: ${formatHHMMFromSeconds(restanteSeg)}`} style={{ width: `${clampPercent(restantePct)}%`, background: 'rgba(148,163,184,0.50)' }} />
        </div>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-2 mt-3">
        <div className="flex items-center gap-2"><span className="w-3 h-3 rounded-full" style={{ background: '#14532d' }} /><span className="text-[11px] font-bold text-[var(--coa-text-soft)]">Deslocamento ({formatHHMMFromSeconds(deslocSeg)})</span></div>
        <div className="flex items-center gap-2"><span className="w-3 h-3 rounded-full" style={{ background: '#22c55e' }} /><span className="text-[11px] font-bold text-[var(--coa-text-soft)]">Produtivo ({formatHHMMFromSeconds(productiveNetSeg)})</span></div>
        <div className="flex items-center gap-2"><span className="w-3 h-3 rounded-full" style={{ background: 'rgba(148,163,184,0.60)' }} /><span className="text-[11px] font-bold text-[var(--coa-text-soft)]">Restante ({formatHHMMFromSeconds(restanteSeg)})</span></div>
      </div>
    </div>
  );
};
  ...
  return (
    <div className="rounded-[14px] border p-3 bg-[rgba(255,255,255,0.015)]" style={{ borderColor: 'var(--coa-divider)' }}>
      <div className="flex items-center justify-between gap-3 mb-2">
        <span className="text-[10px] font-black uppercase tracking-[0.12em] text-[var(--coa-text-soft)]">{label}</span>
        <span className="text-[0.95rem] font-black" style={{ color }}>{formatPercent(value)}</span>
      </div>
      <div className="w-full h-[10px] rounded-full overflow-hidden" style={{ background: 'rgba(255,255,255,0.07)' }}>
        <div className="h-full rounded-full transition-all" style={{ width: `${barWidth}%`, background: color }} />
      </div>
      <span className="block mt-2 text-[11px] font-bold text-[var(--coa-text-muted)]">{getKpiMetaLabel(type)}</span>
    </div>
  );
};
...
const OperationGroupsChart = ({ chartData }) => {
  ...
  return (
    <div className="coa-panel p-4 mt-3 flex flex-col gap-4 bg-[rgba(255,255,255,0.015)] border border-[var(--coa-divider)] rounded-[14px]">
      <div className="flex flex-col gap-1">
...
// ================================= MAIN EXECUTOR -----------------------------------------
const ResumoDetailDiario = ({ selectedDate, setSelectedDate, selectedCategories, setSelectedCategories, availableDates = [] }) => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        // Disparo das Views (Que não sofrem bloqueio do RLS por padrão)
        ...
        //console.log('[COA] 🔍 Buscando tabela crua nas datas:', [selectedBrDate, selectedDate]);

        // AQUI ESTÁ A MÁGICA: Usamos .in() para atirar em ambos os formatos ao mesmo tempo
        // Se retornar 0 linhas aqui, o seu RLS está barrando a leitura! Vá liberar isso, mestre!
        const opTableRes = await supabase
          .from('tb_c_geral')
          .select('*')
          .in('data', [selectedBrDate, selectedDate])
          .limit(10000);
        
        //console.log(`[COA] 📊 Linhas liberadas da tb_c_geral: ${opTableRes.data?.length || 0}`);
  ...
  const areaTableRows = useMemo(() => {
    ...
    areaRowsRaw.filter((row) => selectedCategories.includes(row.categoria)).forEach((row) => {
      const key = row.desc_area || 'NÃO MAPEADO';
      if (!areaMap.has(key)) {
        areaMap.set(key, { desc_area: key, qnt_equip: 0, hrs_total_seg: 0, hrs_disp_seg: 0, hrs_produtivas_seg: 0, hrs_s_apont_seg: 0, ef_op: 0, perc_s_apont: 0 });
      }
  ...
  const compiledChartData = useMemo(() => {
    ...
    // Blindagem de espaços e maiúsculas/minúsculas
    if (selectedAreas.length > 0) {
      const arrAreas = selectedAreas.map(a => a.trim().toUpperCase());
      filteredOpTable = rawOpTable.filter(r => arrAreas.includes((r.desc_area || '').trim().toUpperCase()));
    } else if (selectedCategories.length > 0) {
      const arrAreas = areaTableRows.map(a => (a.desc_area || '').trim().toUpperCase());
      filteredOpTable = rawOpTable.filter(r => arrAreas.includes((r.desc_area || '').trim().toUpperCase()));
    }

    const sums = {
      auxiliar: 0, clima: 0, fabrica_parada: 0, improdutivo: 0, 
      indeterminado: 0, manutencao: 0, produtivo: 0, s_apont: 0, sem_turno: 0
    };
  ...
  }, [rawOpTable, selectedAreas, areaTableRows, selectedCategories]);
        ...
        <div className="coa-card coa-card--resumo-home">
          <div className="coa-card__body flex items-center justify-center">
            <div className="flex flex-col items-center justify-center gap-3 py-10">
              <div className="coa-loader-dots" aria-label="Carregando"><span /><span /><span /></div>
    ...
    return (
      <div className="flex flex-col gap-4">
        ...
        <div className="coa-card">
          <div className="coa-card__body"><div className="coa-empty">{error}</div></div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-4 animate-in fade-in zoom-in-95 duration-300">
      ...
      <AreaFilterTable rows={areaTableRows} selectedAreas={selectedAreas} onToggleArea={handleAreaToggle} />
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[minmax(0,1.15fr)_320px] gap-3 items-start">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
          ...
          <MetricCard label="Produtivas" value={formatHHMMFromSeconds(totals.hrs_produtivas_seg)} color="var(--coa-success)" />
          <MetricCard label="Deslocamento" value={formatHHMMFromSeconds(totals.hrs_deslocamento_seg)} color="var(--coa-text)" />
        </div>
        <div className="flex flex-col gap-3">
      ...
      <div className="grid grid-cols-1 gap-3">
        <BreakdownBar totalSeg={totals.hrs_total_seg} deslocSeg={totals.hrs_deslocamento_seg} productiveNetSeg={totals.hrs_produtivo_liquido_seg} restanteSeg={totals.hrs_restante_seg} />
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          <KpiProgressCard label="Disponibilidade Mecânica" value={totals.disp_mec} type="disp_mec" />
        </div>
      </div>
