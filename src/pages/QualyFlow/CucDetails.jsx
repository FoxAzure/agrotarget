...
const CucDetails = () => {
  ...
  const getLoteCardStyle = (cuc) => {
    if (cuc >= 90) return 'border-green-300 bg-green-50/60 hover:bg-green-100/60';
    if (cuc >= 80) return 'border-yellow-300 bg-yellow-50/60 hover:bg-yellow-100/60';
    return 'border-red-300 bg-red-50/60 hover:bg-red-100/60';
  };
              ...
              <div key={`detalhes-${activeIdx}`} className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {activeCampo.lotes.map((lote, i) => (
                    <button
                      ...
                      className={`flex flex-col items-center p-2 rounded-xl border transition-all text-center relative overflow-hidden ${getLoteCardStyle(lote.cuc)}`}
                    >
                      <span className="text-[8px] font-black text-slate-500 uppercase mb-1">Lote: {lote.nome}</span>
                      <span className={`text-xl font-black tracking-tighter mb-2 ${getCucColor(lote.cuc)}`}>{lote.cuc.toFixed(1)}%</span>
                      
                      <div className="w-full flex justify-between px-1 text-[8px] font-black border-t border-slate-200/60 pt-1.5 uppercase">
                        <span className={getVazaoColor(lote.vazao)}>{lote.vazao.toFixed(2)}L/h</span>
                ...
                <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex flex-col gap-4">
                  ...
                  <div className="flex bg-slate-100 p-1 rounded-lg">
                    <button onClick={() => setChartTab('cuc')} className={`flex-1 text-[9px] font-black uppercase py-1.5 rounded-md transition-all ${chartTab === 'cuc' ? 'bg-white shadow-sm text-agro-green' : 'text-slate-400 hover:text-slate-500'}`}>CUC</button>
                    <button onClick={() => setChartTab('vazao')} className={`flex-1 text-[9px] font-black uppercase py-1.5 rounded-md transition-all ${chartTab === 'vazao' ? 'bg-white shadow-sm text-agro-green' : 'text-slate-400 hover:text-slate-500'}`}>Vazão</button>
                    <button onClick={() => setChartTab('entup')} className={`flex-1 text-[9px] font-black uppercase py-1.5 rounded-md transition-all ${chartTab === 'entup' ? 'bg-white shadow-sm text-agro-green' : 'text-slate-400 hover:text-slate-500'}`}>Entupidos</button>
                  </div>
                  ...
                  <div className="flex flex-col gap-2.5">
                    {(() => {
                      ...
                      return activeCampo.lotes.map((lote, idx) => {
                        ...
                        return (
                          <div key={`bar-${idx}`} className="flex flex-col gap-1">
                            <div className="flex justify-between items-end">
                              <span className="text-[10px] font-black text-slate-600 uppercase tracking-wider">
                                Lote {lote.nome}
                              </span>
                              <span className="text-[10px] font-black tracking-tighter" style={{ color: barColor }}>
                                {barText}
                              </span>
                            </div>
            ...
            <div className="p-5">
              ...
              <div className="flex justify-between items-center bg-slate-50 p-4 rounded-2xl border border-slate-100">
                 <div className="flex flex-col">
                    <span className="text-[8px] font-black text-slate-400 uppercase">CUC %</span>
