...
const COAAreaCard = ({ areaName, stats, to }) => {
  const { 
    qtdEquipamentos = 0,
    totalHoras = 0,
    horasDisponiveis = 0,
    horasProdutivas = 0,       // <-- Adicionado
    horasSemApontamento = 0,   // <-- Adicionado
    horasMotor = 0,
    horasOcioso = 0,
    percSemApontamento = 0,
    percIndeterminado = 0,
    percMotorOcioso = 0,
  ...
  return (
    // CARD MAIS "GORDINHO": max-w-[400px]
    <section className="w-full max-w-[400px] animate-in fade-in zoom-in duration-500 mt-2 font-sans">
      <div className="bg-[#1A202C] border-2 border-slate-700/80 hover:border-emerald-500/60 rounded-xl shadow-2xl overflow-hidden group transition-all relative">
        
        <div className="absolute top-0 left-0 w-full h-1.5 bg-emerald-500 opacity-80 shadow-md group-hover:opacity-100 transition-opacity" />

        {/* Header Compacto */}
        <div className="px-5 pt-4 pb-3 flex justify-between items-start border-b border-slate-700/80 bg-[#11151D]">
          <h2 className="text-[13px] font-black uppercase tracking-widest text-slate-100">
            {areaName}
          </h2>
          <span className="text-[10px] font-bold text-slate-300 bg-[#0A0D14] px-2.5 py-1 rounded border border-slate-600 shadow-sm">
            {qtdEquipamentos} EQUIP
          </span>
        </div>
        ...
        <div className="flex p-4 gap-4">
          ...
          {/* Direita: Métricas Brutas (GRID DE 6 ITENS) */}
        ...
        {/* Bloco Central: Barras de Progresso */}
        ...
        {/* Rodapé: Top 3 Ofensores */}
        {ofensores && ofensores.length > 0 && (
          <div className="px-5 py-3.5 bg-[#11151D]/80">
            <span className="text-[9px] font-black text-red-400 uppercase tracking-widest mb-2 block">Principais Ofensores</span>
        ...
        >
          <div className="flex items-center gap-2 opacity-80 group-hover:opacity-100">
            <span className="text-[10px] font-black uppercase tracking-widest group-hover:text-emerald-400 text-slate-300">
              Relatório Detalhado
            </span>
